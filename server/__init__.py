...
@router.post("/create")
async def handle_server_create(
    slug: str,
    data: Annotated[TokenData, Security(get_token, scopes=Permission.SERVER_CREATE)],
):
...
async def handle_server_list(data: TokenData = Depends(token_data)):
    servers = await Server.select(Server.slug, Server.state, Server.uuid).where(
        Server.owner.username == data.username
    )
    return success_short(data=servers)


server_start_responses = {
    200: {"description": "Success", "model": APIResponse},
    403: {"description": "Invalid Permissions", "model": APIResponse},
    404: {"description": "Server not found", "model": APIResponse},
    409: {"description": "Invalid server state", "model": APIResponse},
}
...
async def handle_server_start(slug: str, data: TokenData = Depends(token_data)):
    server = (
        await Server.select(Server.uuid, Server.state, Server.owner.username)
        .where(Server.slug == slug)
        .first()
    )
    if server is None:
...
async def handle_server_stop(slug: str, data: TokenData = Depends(token_data)):
    server = (
        await Server.select(Server.run_id, Server.owner.username)
        .where(Server.slug == slug)
        .first()
    )
    if server is None:
    ...
    if not (server["owner.username"] == data.username):
        return error_short("you do not own this server", 403)
    await Server.update(state=State.STOPPED, run_id=None).where(Server.slug == slug)
    try:
