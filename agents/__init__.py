...
async def login_callback(message: AbstractIncomingMessage):
    ...
    if agent is None:
        ...
        await message.channel.basic_publish(
            response.model_dump_json().encode(), routing_key=message.reply_to
        )
        return
    print(f"Logged in as {agent.agent_name}")
    queue = await message.channel.queue_declare(
        durable=True, auto_delete=True, exclusive=False
    )
    await message.channel.queue_bind(queue.queue, "control", queue.queue)
    response = LoginResponsePacket(
        Success=True, Name=agent.agent_name, Queue=queue.queue
    )
...
async def handle_agent_create(
    agent_name: str,
    token: Annotated[TokenData, Security(get_token, scopes=[Permission.AGENT_CREATE])],
):
    ...
    return success_short(
        "agent successfully created", token=token, agent_name=agent_name, code=201
    )


@router.get("/list")
async def handle_agent_list(
    token: Annotated[TokenData, Security(get_token, scopes=[Permission.AGENT_LIST])],
):
    names = [
        dict(name=agent["agent_name"], last_ping=agent["last_ping"])
